...
public class GOAnimTransformModify: MonoBehaviour
{
    ...
    public UISoftwareTransformAnimModify GetData()
    {
        return m_localAnim == null ? m_presetAnim : m_localAnim;
    }

    public void Update()
    {
        if (m_localAnim.m_status != UISoftwareAnimStatus.CompleteRun && m_localAnim.m_status != UISoftwareAnimStatus.CompleteReverse)
        {
            var l_vec2 = m_localAnim.Update();
            m_target.localPosition = new Vector3(l_vec2.x, l_vec2.y, m_target.localPosition.z);
        }
    }
